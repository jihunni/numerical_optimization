function [x, y, x_all_iteration, y_all_iteration] = nonlinear_conjugate_gradient_method_PR(objective_function, x, y, termination_condition)
%nonlinear_conjugate_gradient_method_Polak-Ribiere (CG-PR) Summary of this function goes here
...
%            y_all_iteration : (list) iterative history of y (for plot)
 % to measure the performance
    ...
    while(transpose(gradient) * gradient >= termination_condition)
        ...
        beta = (transpose(gradient) * (gradient - gradient_old)) / (transpose(gradient_old) * gradient_old) ;
        P = -gradient + beta * P ;
    ...
    fprintf('--------------------------------------------\n')
    fprintf('the final (x, y) in nonlinear conjugate gradient method (PR): (%i, %i)\n', x, y);
